...
class View(discord.ui.View):
	...
	def add_buttons(self):
		...
		self.backB = Buttons(emoji="◀️", style=discord.ButtonStyle.blurple, command=self.to_page, args=-1, ctx = self.ctx)
		self.nextB = Buttons(emoji="▶️", style=discord.ButtonStyle.blurple, command=self.to_page, args=+1, ctx = self.ctx)
		self.endB = Buttons(emoji="⏩", style=discord.ButtonStyle.grey, command=self.set_page, args=len(self.options)-1, ctx = self.ctx)
		self.quitB = Buttons(emoji="⏹", style=discord.ButtonStyle.red, command=self.quit, ctx = self.ctx)
	...
	def gen_embeds(self):
		options, embeds = [], []
		options.append(discord.SelectOption(label="Home", description="Show the home page.", emoji='<:1_folder:1012950710008152104>'))
		embeds.append(self.home)
		for cog in self.get_cogs():
			if "help_custom" in dir(cog):
				emoji, label, description = cog.help_custom()
				options.append(discord.SelectOption(label=label, description=description, emoji=emoji))
				embed = discord.Embed(title = f"{emoji} Help · {label}",description=f"`{cog.__doc__}`", url="https://discord.gg/Zfz4YKRmSd")
